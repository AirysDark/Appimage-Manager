...
LOG_FILE="$HOME/.appimage_manager.log"
INSTALL_DIR="$HOME/.local/bin"
SCRIPT_NAME="appimage_manager.sh"
REPO_URL="https://github.com/yourusername/appimage-manager"

install_dependencies() {
    if ! command -v zenity &>/dev/null; then
        echo "Zenity is not installed. Installing it now..."
        sudo apt update && sudo apt install -y zenity || {
            echo "Failed to install Zenity. Please install it manually." >&2
            exit 1
        }
    fi
}
...
install_appimage() {
    APPIMAGE_FILE=$(zenity --file-selection --title="Select AppImage File" --filename="$HOME/")
    [[ -z "$APPIMAGE_FILE" ]] && return
...
[Desktop Entry]
Name=$APP_NAME
Exec=$APP_DIR/$(basename "$APPIMAGE_FILE")
Icon=$ICON
Type=Application
Categories=Utility;
Terminal=false
...
uninstall_appimage() {
    APP_LIST=$(ls "$APP_DIR_ROOT" 2>/dev/null)
    [[ -z "$APP_LIST" ]] && zenity --info --text="No AppImages found in /opt" && return
...
list_appimages() {
    APPS=$(ls "$APP_DIR_ROOT" 2>/dev/null)
    [[ -z "$APPS" ]] && zenity --info --text="No AppImages installed." && return
...
main_menu() {
    while true; do
        ACTION=$(zenity --list --title="AppImage Manager" \
...
install_from_github() {
    mkdir -p "$INSTALL_DIR"
    curl -L "$REPO_URL/raw/main/appimage_manager.sh" -o "$INSTALL_DIR/$SCRIPT_NAME"
    chmod +x "$INSTALL_DIR/$SCRIPT_NAME"
